...
export interface IArtSpermDonor extends IArtDonor {}

const ArtSpermDonorSchema = new Schema<IArtSpermDonor>(
  {
    donorCode: { type: String, required: true, unique: true, index: true },
    gameteType: { type: String, default: "sperm", index: true },
    availability: {
      type: String,
    ...
    availabilityLabel: { type: String, default: "Available Immediately (Cryopreserved)" },
    age: { type: Number, required: true, min: 21, max: 55 },
    ...
    bodyBuild: { type: String, default: "Medium / Athletic" },
    ...
    provenFertility: { type: String, default: "Proven sperm motility / Father of healthy child" },
    ...
    geneticScreenings: {
      thalassemia: {
        ...
        hba2Fraction: { type: String, default: "2.3%" },
      },
      karyotype: {
        result: { type: String, default: "46,XY (Normal Male)" },
  ...
  {
    timestamps: true,
    collection: "art_sperm_donors",
  }
);

export const ArtSpermDonor =
  mongoose.models?.ArtSpermDonor || mongoose.model<IArtSpermDonor>("ArtSpermDonor", ArtSpermDonorSchema);
