    ...
    <Document title={`Donor_Registration_${registrationId || "Draft"}`}>
      ...
      {showSection("registration") && (
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.table}>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}>
                ...
                <Text>{DField(consent.signatureDate)}</Text>
              </View>

              <View style={styles.tableCell}>
                <Text style={styles.label}>Sample ID</Text>
                <Text>{DField("[System Gen]")}</Text>
              </View>

              <View style={styles.tableCell}>
                <Text style={styles.label}>Donor ID</Text>
                <Text>{DField(registrationId || "MAB/SD/___")}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={[styles.list, styles.htext]}>
            {[
              {
                en: `My date of birth age ${personalInfo.dateOfBirth}, as on today is more than twenty-one years and less than fifty-five years.`,
                hi: `मेरी जन्मतिथि ${personalInfo.dateOfBirth} है, आज के हिसाब से मेरी उम्र इक्कीस साल से ज़्यादा और पचपन साल से कम है।`,
              },
              {
      ...
      {showSection("contract") && (
        <Page size="A4" style={styles.page}>
          ...
          <Text style={[styles.text, { maxWidth: 350, alignSelf: "center" }]}>
            ...
            the {DField(day)} day of {DField(month)}, (year), in {DField(year)}{" "}
      ...
      {showSection("certificate") && (
        <Page size="A4" style={styles.page}>
          ...
          <Text style={styles.text}>
            We hereby certify that Mr {DField(personalInfo.fullName)} age{" "}
            {DField(personalInfo.age)} years Aadhar Card No.{" "}
            {DField(personalInfo.aadhaarNumber)} is registered Sperm donor of
            MEDIYAZ ART BANK (ART Bank Registration No.
            DL/AB/2022/10605/AB/SEB/21) Donor registration No.{" "}
            {DField(registrationId || "MAB/SD/___")} and his Records have been
      ...
      {showSection("affidavit") && <AffidavitPdf />}

      {showAttachment("stamp") && (
        <Page size="A4" style={styles.page}>
          {withHeader && (
            <Image
