...
export async function GET(
  ...
  try {
    ...
    const donor = await ArtEggDonor.findById(id);
    if (!donor) {
      return NextResponse.json({ success: false, error: "Egg donor not found" }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    const updated = await ArtEggDonor.findByIdAndUpdate(id, body, {
      new: true,
      runValidators: true,
    });
    ...
    console.error("PUT /api/art-donors/egg/[id] error:", err);
    return NextResponse.json({ success: false, error: err.message }, { status: 500 });
  }
}

// DELETE: Delete egg donor
export async function DELETE(
  ...
  try {
    ...
    await connectToDatabase();
    const { id } = await params;
    const deleted = await ArtEggDonor.findByIdAndDelete(id);
    if (!deleted) {
      return NextResponse.json({ success: false, error: "Egg donor not found" }, { status: 404 });
    }
    ...
    return NextResponse.json({ success: true, message: "Egg donor deleted successfully." });
  } catch (err: any) {
    console.error("DELETE /api/art-donors/egg/[id] error:", err);
    return NextResponse.json({ success: false, error: err.message }, { status: 500 });
  }
}
