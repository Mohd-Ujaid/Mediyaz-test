...
import { EggDonor } from "@/models/EggDonor";
...
export async function GET(req: Request) {
  try {
    ...
    if (applicationId) {
      const donor = await EggDonor.findOne({ applicationId });
      ...
      const sanitized = {
        ...
        personalInfo: {
          fullName: donor.personalInfo?.fullName ? donor.personalInfo.fullName.replace(/(?<=.).(?=.)/g, "*") : "Egg Donor Applicant",
    ...
    const donors = await EggDonor.find({}).sort({ createdAt: -1 });
...
export async function POST(req: Request) {
  try {
    ...
    const donor = await EggDonor.create({ ...body, applicationId, status: "PENDING_REVIEW" });
...
export async function PATCH(req: Request) {
  try {
    ...
    const donor = await EggDonor.findByIdAndUpdate(id, { $set: updateFields }, { new: true });
...
export async function DELETE(req: Request) {
  try {
    ...
    await EggDonor.findByIdAndDelete(id);
