...
export default function AdminRequirementDetails() {
  ...
  const handleSendWhatsAppSimulated = () => {
    ...
    const message = `Hello ${reqData.personalDetails.fullName}, this is the Clinical Coordinator from Mediyaz Art Bank. We received your donor requirement. Let us know a convenient time to schedule your consultation.`;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4 border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-450 mt-1 font-mono">Requirement ID: {reqData._id} • Submitted: {new Date(reqData.createdAt).toLocaleString()}</p>
          </div>
        </div>
                  ...
                  <div key={file.fileId} className="flex justify-between items-center p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 hover:bg-slate-50 transition-all text-xs">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setViewerDoc({
                          isOpen: true,
                          title: file.name || "Medical Attachment",
                          fileUrl: file.url,
                          fileName: file.name,
                        })
                      }
                      className="rounded-lg h-8 px-2 text-primary hover:bg-primary/5 cursor-pointer"
